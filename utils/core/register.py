    ...
    while True:
        ...
        while True:
            ...
            if proxy:
                ...
                ip_list = (client_proxy["hostname"]).split(".")
                ip_peace = f"{ip_list[0]}.{ip_list[1]}.{ip_list[2]}"
                with open('sessions/accounts.json', 'r') as file:
                    data = str(json.load(file))
                if ip_peace in data:
                    print('Proxy in same subnet already in accounts.json')
                elif str(client_proxy['hostname']) in data:
                    print('Proxy already in accounts.json')
                else:
                    break
            else:
